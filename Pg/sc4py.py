...
def preprocess_data(transactions):
    ...
    return np.array(features), np.array(labels), user_transaction_ids
...
def visualize_graph_with_predictions(G, predictions):
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('suspicious_activity_network_xgboost_with_transactions.png', dpi=300, bbox_inches='tight')
    print("Graph visualization saved as 'suspicious_activity_network_xgboost_with_transactions.png'")
...
def main(file_path):
    ...
    X, y, user_transaction_ids = preprocess_data(transactions)
    ...
    visualize_graph_with_predictions(G, y_pred)
